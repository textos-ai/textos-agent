...
## CRITICAL: NEVER USE STR_REPLACE ON ASTRO FILES

NEVER use str_replace, the Edit tool, or any text-match
replacement on .astro files. These files are large and
contain base64 images, SVG paths, and special characters
that cause str_replace to corrupt files silently.

For ALL .astro file edits use ONLY PowerShell
line-number replacements:

  $lines = Get-Content "path\to\file.astro"
  $lines[LINE_NUMBER - 1] = 'new content'
  Set-Content "path\to\file.astro" $lines -Encoding UTF8

Before every .astro edit:
  1. Find the exact line number first using grep or search
  2. Use PowerShell line replacement only
  3. Verify line count after every change:
     (Get-Content "path\to\file.astro").Count
  4. If line count drops unexpectedly run immediately:
     git restore "path\to\file.astro"

This rule applies to ALL .astro files without exception:
live.astro, builder.astro, index.astro, and any other
.astro file in the project.
...
This repo is the **backend agent** â€” a Cloudflare Worker built on
...
3. Owns Supabase reads/writes via the service-role key (bypasses RLS
   by design â€” frontend reads use anon key + JWT and hit RLS as
   `authenticated`).
...
**Founder:** Rob Gaudet â€” see `../textos-web/CLAUDE.md` for full
...
- **Cloudflare Workers** â€” runtime; `wrangler` for deploy.
- **Hono** â€” HTTP router + middleware (CORS, error handlers).
- **Anthropic Agent SDK** â€” Claude Sonnet 4.6 (primary), Haiku 4.5
  (lightweight), Opus 4.7 (premium). Routing in
  `src/agent/model-router.ts`.
- **Supabase Postgres + Auth** â€” `users`, `businesses`, `tasks`,
  `task_runs`, `subscription_plans`, `user_subscriptions`,
  `task_purchases`.
- **Cloudflare D1** â€” per-business activity logs + cache. Sprint 3+;
  commented placeholder in `wrangler.toml`.
- **Cloudflare Queues** â€” long-running task dispatch. Sprint 5+.
- **Cloudflare KV** â€” rate limits, magic-link tokens, agent prompt
  cache. Sprint 3+.
- **Cloudflare R2** â€” generated documents, hero images, exports.
  Sprint 7+.
- **Stripe** â€” subscriptions + Stripe Connect. Sprint 6.
- **SendGrid** â€” magic links + outbound email. Sprint 3+.

### Endpoints (Sprint 2)

- `GET  /healthz` and `GET /version` â€” liveness, JSON payload.
- `POST /chat` â€” Zod-validated body `{message, model?}`. Streams
  Anthropic SSE events end-to-end.
- `POST /tasks/run` â€” Zod-validated `{taskSlug, businessId?, inputs?}`.
...
- `area` â€” `business_builder` | `daycycle` | `personal_website` |
  `public_business_website` | `business_manager`
- `is_default` â€” runs automatically during free build
- `plan_required` â€” `free` | `core_paid` | `premium_only` |
  `premium_inactive`
- `visibility` â€” `hidden` | `teaser_locked` | `fully_locked` |
  `always_visible`
- `price_cents`, `prompt_template`, `output_type`, `inputs_required`,
  `status`

**Adding a task is a database write â€” no Worker code change.** This
...
```
src/
â”œâ”€â”€ index.ts          â† Hono app, CORS, route mount, 404 + onError
â”œâ”€â”€ env.ts            â† Env interface (secrets + bindings)
â”œâ”€â”€ routes/
â”‚   â”œâ”€â”€ health.ts     â† /healthz, /version
â”‚   â”œâ”€â”€ chat.ts       â† /chat (SSE)
â”‚   â””â”€â”€ tasks.ts      â† /tasks/run
â”œâ”€â”€ services/
â”‚   â”œâ”€â”€ anthropic.ts  â† createAnthropicClient + chatWithClaude
â”‚   â””â”€â”€ supabase.ts   â† createSupabaseClient + getTaskBySlug + getActivePlans
â”œâ”€â”€ agent/
â”‚   â”œâ”€â”€ loop.ts       â† Sprint 2 stub; expanded in Sprint 5
â”‚   â””â”€â”€ model-router.ts â† MODEL_IDS + pickModel(taskType?, complexity?)
â””â”€â”€ lib/
    â”œâ”€â”€ errors.ts     â† Typed ErrorBody helpers
    â””â”€â”€ logger.ts     â† Structured JSON logger

supabase/
â””â”€â”€ migrations/
    â”œâ”€â”€ 20260429120000_initial_schema.sql â† 7 tables, 8 enums, RLS
    â””â”€â”€ 20260429120001_seed_data.sql      â† 4 plans, 30 tasks
```
...
- **Sprint 1 â€” Astro scaffold âœ…** (in `textos-web`) â€” closed 2026-04-28
- **Sprint 2 â€” TextOS Agent foundation + tasks data model âœ…**
  (this repo) â€” closed 2026-04-29
- **Sprint 3 â€” Supabase Auth + handle DNS** (next)
  Google Sign-In + magic link via Supabase Auth, handle picker,
  Cloudflare DNS API for `{handle}.app.textos.ai`, JWT round-trip
  frontend â†” Worker.
- **Sprint 4 â€” Business Builder dashboard shell**
  5-column layout reads from real `tasks` table.
- **Sprint 5 â€” Free build flow + existing-business entry path**
  Three entry paths (idea / find-me-one / I-have-a-business). Real
  agent loop replaces the Sprint 2 stub.
- **Sprint 6 â€” Stripe + four-tier paywall**
  Webhook handler, Founders cohort counter (1,000 cap), Ã  la carte
  purchase flow.
- **Sprint 7 â€” 8 paid-bundle tasks**
...
in any of the following core tables:
  ...
  tasks              â€” task names, slugs, descriptions, order
  task_runs          â€” run state, output, status
  business_assets    â€” asset types, subtypes, content
  businesses         â€” business names, slugs, metadata
  business_context   â€” agent name, industry, summary, market data
  business_subscriptions â€” subscription status, plan details
  users              â€” user data, tier, handle
  token_balances     â€” token counts, periods
  token_transactions â€” transaction history
...
WHAT TO DO INSTEAD:
  - task names â†’ read tasks.name from the DB join
  - task order â†’ read tasks.execution_order from the DB
  - document names â†’ read tasks.name via asset.task_run_id join
  - lifecycle phases â†’ read tasks.lifecycle_phase_id join
  - free build tasks â†’ query tasks WHERE is_default = true
  - asset display names â†’ read tasks.name from matching task_run

BEFORE CREATING ANY CONSTANT LIST:
  1. Check the database schema â€” is this data already in a table?
...
ACCEPTABLE CONSTANTS (not DB data):
  - CSS class names / rendering discriminator maps
    (e.g. slug â†’ which CSS template to apply for modal rendering)
...
See `../textos-web/CLAUDE.md` Â§ Non-Negotiable Rules. All 13 apply
...
ease â€” prefer proper tooling and durable solutions over manual
cycles.**

(b) **Hates being a copy-paste mule â€” Claude should do as much work
...
WebForms, LINQ-to-SQL, SQL Server) â€” NOT a CLI/bash/Linux user.
...
- **Cloudflare Workers + Anthropic Agent SDK + Hono** for the agent
  ...
  for full reasoning.
- **Tasks are first-class database entities** â€” catalog is data, not
...
- âœ… Cloudflare Worker `textos-agent-dev` deployed
- âœ… Live URL: `https://textos-agent-dev.rgaudet2023.workers.dev`
- âœ… 4 endpoints (`/healthz`, `/version`, `/chat`, `/tasks/run`)
- âœ… Supabase schema + seeds applied (4 plans + 30 tasks)
- âœ… `tsc --noEmit` passes with zero errors
- âœ… All smoke tests green:
  - `/healthz` â†’ HTTP 200, ~290ms
  - `/version` â†’ HTTP 200, ~120ms
  - `/chat` (Haiku stream) â†’ HTTP 200, TTFB ~100ms
  - `/tasks/run welcome-email` â†’ HTTP 200, ~980ms
  - `/tasks/run does-not-exist` â†’ HTTP 404
- âœ… Closed-source private repo on GitHub

---

## ROLES â€” CLAUDE vs CLAUDE CODE
...
- Env vars via define:vars block â†’ expose as window globals
...
1. LOCAL CLAUDE.md FILES
   ...
   Start with business routing â€” caused 7 hours of pain on May 16.
...
================================================================
## PROJECT CONTEXT â€” WHAT WE ARE BUILDING
================================================================
...
budget. TextOS handles everything else â€” forever.
...
WHAT IS BUILT AND WORKING:
  âœ“ Authentication (Google OAuth + magic link, no passwords)
  âœ“ Business Builder (10-task free build pipeline, SSE streaming)
  âœ“ Generated business websites (static, deployed to {slug}.app.textos.ai)
  âœ“ Token economy (30 tokens/period, top-up bundles, ledger)
  âœ“ Stripe subscriptions ($49.99/mo Founders, $29.99/mo Standard)
  âœ“ Stripe Connect schema (businesses table has stripe_connect_* fields)
  âœ“ Operator School (6 badges, 12 lessons)
  âœ“ Business Manager V1 (goals, charge windows, milestones)
  âœ“ Business Live page (/business/live â€” operations view)
  âœ“ Admin portal (Phase 1+2)
  âœ“ Marketing: carousel generator, cold email, social content plan

WHAT IS COMING NEXT (Autonomous Business Engine):
  â†’ DIGITAL_PRODUCT generation pipeline (PDF via Claude + R2 storage)
  â†’ Stripe paywall on business website (/buy + /download pages)
  â†’ Ad creative generation (Meta first, then Google, TikTok)
  â†’ Ad platform API integrations
  â†’ Nightly optimization loop (Cron Trigger at 2am UTC)
  â†’ Event-driven threshold workers (Cloudflare Queues)
  â†’ Ad wallet funding flow (Stripe â†’ TextOS â†’ platforms)
  â†’ Performance reporting dashboard (metrics to activator)

NET NEW TABLES FOR AUTONOMOUS ENGINE (not yet created):
  business_products     â€” digital product assets, R2 file location, pricing
  ad_campaigns          â€” campaign config, platform, status, daily budget
  ad_creatives          â€” creative assets, platform format, performance data
  ad_wallets            â€” activator balance, disbursement history, fee ledger
  performance_snapshots â€” daily metrics: revenue, ROAS, CVR, ad spend
...
Do NOT do this migration mid-feature â€” do it as a standalone task.
...
Rules:
  ...
  - Throw on error â€” never return error objects
  - Named exports only â€” no default exports from service files
...
Rules (non-negotiable):
  ...
  - Activator manually edits field â†’ source="human_edited", locked=true
  - Optimization loop reads locked BEFORE any write
  - locked=true â†’ skip this field, write recommendation to dashboard
  - locked=false â†’ optimization loop may update, source="ai_optimized"
...
================================================================
## AUTONOMOUS ENGINE â€” ARCHITECTURE CONSTRAINTS
================================================================

PDF GENERATION:
  Headless browsers (Puppeteer/Chrome) CANNOT run in Cloudflare
  Workers â€” no DOM environment. Use an external PDF API called
...
AD WALLET CONSTRAINT:
  The ad wallet funding flow (Stripe â†’ TextOS â†’ platforms) requires
  legal clearance before enabling real money movement.
  Build the schema, UI, and disbursement logic â€” but gate actual
...
EVENT-DRIVEN TRIGGERS (Cloudflare Queues, fire immediately):
  payment_intent.succeeded    â†’ generate signed URL, send email
  ad_spend_threshold breach   â†’ pause ALL campaigns immediately
  roas_critical (< 0.5/48h)  â†’ pause campaign, queue new creative
  conversion_rate_drop > 50%  â†’ check technical, flag for nightly
  wallet_low (< 20% budget)   â†’ alert activator, pause if < $10

================================================================
## AI-GENERATED TESTIMONIALS â€” PROHIBITED
================================================================
